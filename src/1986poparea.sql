...
DROP INDEX IF EXISTS in_1986_vor_ea_geom_idx; 
CREATE INDEX in_1986_vor_ea_geom_idx ON in_1986_vor_ea USING GIST (geom);
...
CREATE TABLE x_source_blocks_clipped_all AS (
    ...
    (WITH J AS (SELECT 
        ...
        in_1986_vor_ea.ctuid,
        in_1986_vor_ea.geom 
        FROM in_1986_vor_ea
        LEFT JOIN x_source_blocks_clipped ON
        in_1986_vor_ea.id = x_source_blocks_clipped.id
        WHERE in_1986_vor_ea.ctuid IS NOT NULL)
...
WITH db_pop_ctuid AS 
    ...
    in_1986_vor_ea.ctuid,
    in_1986_vor_ea.id,
    in_1986_vor_ea.pop AS db_pop,
    in_1986_vor_ea.dwe AS db_dwe
    FROM in_1986_vor_ea
    WHERE in_1986_vor_ea.ctuid IS NOT NULL),
db_ct_op AS 
    (SELECT 
    ...
    pop_ct_1986.ct_pop,
    pop_ct_1986.ct_dwe
    FROM db_pop_ctuid LEFT JOIN pop_ct_1986 
    ON db_pop_ctuid.ctuid = pop_ct_1986.ctuid
    ORDER BY ctuid)
SELECT
...
-- join to the target tracts - this case to 1991
DROP TABLE IF EXISTS x_source_target;
CREATE TABLE x_source_target AS (
    SELECT
    x_source_blocks_clipped_ready.id AS source_dbuid,
    x_source_blocks_clipped_ready.ctuid AS source_ctuid,
    in_1991_cbf_ct_moved_clipped.geosid AS target_ctuid,
    x_source_blocks_clipped_ready.db_pop,
    x_source_blocks_clipped_ready.db_dwe,
    x_source_blocks_clipped_ready.ct_pop + 0.001 AS ct_pop,
    x_source_blocks_clipped_ready.ct_dwe + 0.001 AS ct_dwe,
    x_source_blocks_clipped_ready.db_area + 0.001 AS db_area,
    ST_Intersection(ST_MakeValid(x_source_blocks_clipped_ready.geom),ST_MakeValid(in_1991_cbf_ct_moved_clipped.geom)) AS geom
    FROM x_source_blocks_clipped_ready LEFT JOIN in_1991_cbf_ct_moved_clipped
    ON ST_Intersects(ST_MakeValid(x_source_blocks_clipped_ready.geom),ST_MakeValid(in_1991_cbf_ct_moved_clipped.geom))
    WHERE x_source_blocks_clipped_ready.geom && in_1991_cbf_ct_moved_clipped.geom
);
