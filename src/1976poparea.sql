...
DROP INDEX IF EXISTS in_1976_cbf_ct_geom_idx; 
CREATE INDEX in_1976_cbf_ct_geom_idx ON in_1976_cbf_ct USING GIST (geom);

DROP INDEX IF EXISTS in_1976_gaf_pt_adjust_geom_idx; 
CREATE INDEX in_1976_gaf_pt_adjust_geom_idx ON in_1976_gaf_pt_adjust USING GIST (geom);

-- create min gaf file with CT
DROP TABLE IF EXISTS in_1976_gaf_pt_min;
CREATE TABLE in_1976_gaf_pt_min AS (
	SELECT 
	in_1976_gaf_pt_adjust.prov, in_1976_gaf_pt_adjust.fed, in_1976_gaf_pt_adjust.ea, in_1976_gaf_pt_adjust.pop, in_1976_gaf_pt_adjust.hhlds, 
	in_1976_cbf_ct.geosid AS ctuid,
	in_1976_gaf_pt_adjust.geom
	FROM in_1976_gaf_pt_adjust
	LEFT JOIN
...
CREATE TABLE pop_ct_1976 AS (
	SELECT
	in_1976_cbf_ct.geosid AS ctuid,
	coalesce(sum(in_1976_gaf_pt_min.pop::integer),0) AS ct_pop,
	coalesce(sum(in_1976_gaf_pt_min.hhlds::integer),0) AS ct_dwe
	FROM in_1976_gaf_pt_min
	LEFT JOIN in_1976_cbf_ct ON in_1976_gaf_pt_min.ctuid = in_1976_cbf_ct.geosid
	GROUP BY in_1976_cbf_ct.geosid 
);
